...
class Transaction(MessageObject):
    ...
    def __init__(self, buffer: bytes=None, type: int=0, timestamp: int=0, amount: int=0,
                 ...
                 sender_identifier: bytes=None, sender_data: bytes=None, signature: bytes=None, balance_list_cycle_transaction: bool=False, app_log=None):
    ...
    def get_fee(self):
        if self._type in [self.type_cycle, self.type_cycle_signature]:
            return 0(identifier, cycle_signature)
    ...
    def to_json(self) -> str:
        if self._type == self.type_coin_generation:
            ...
            return json.dumps({"message_type": "Transaction", 'value': {
                ...
                'sender_data': self._sender_data.hex(), 'signature': self._signature.hex(), 'vote': self._cycle_transaction_vote, 'cycle_transaction_signature': self._cycle_transaction_signature.hex()}})
