...
def test_status_1(verbose=False):
    ...
    message2 = StatusResponse(buffer=message.get_bytes())  # 10 more bytes to account for timestamp + type
    if verbose:
        print(message2.get_lines())
    assert str(message2.get_lines()) == "['line1', 'line2']"


def test_blockrequest(verbose=False):
    ...
    message2 = BlockRequest(buffer=message.get_bytes())  # 10 more bytes to account for timestamp + type
    if verbose:
